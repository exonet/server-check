...
def test_00_check_config(domain):
    with patch('subprocess.Popen') as popen:
        popen.return_value.communicate.return_value = [b'config ok']

        assert "PHP config does not contain 'error' or 'warning'" in php.check_config()

        popen.return_value.communicate.return_value = [b'error and warning']
...
@patch('server_check.php.open')
@patch('os.chown')
def test_02_test_mod_ruid2(mock_chown, mock_open, domain):
